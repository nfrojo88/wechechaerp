...
@foreach($replenishments as $rep)
    ...
    <div class="modal fade" id="reviewModal{{ $rep->id }}" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
            <form method="POST" action="{{ url('/assigned-accounts/' . $rep->chart_of_account_id . '/replenishments/' . $rep->id . '/send-to-audit') }}" class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
                @csrf
                <div class="modal-header text-white py-3 px-4 flex-shrink-0" style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); border-bottom: 3px solid #f59e0b;">
                    <div>
                        <h5 class="modal-title fw-bold mb-0 text-white d-flex align-items-center gap-2">
                            <span class="badge bg-warning-subtle text-warning border border-warning border-opacity-25 px-2.5 py-1 rounded-pill fs-6"><i class="fa-solid fa-magnifying-glass-dollar me-1"></i> Review Vouchers</span>
                            <span>Route to Audit #{{ $rep->request_no }}</span>
                        </h5>
                        <small class="text-white-50 d-flex flex-wrap gap-2 align-items-center mt-1">
                            ...
                            <span>&bull;</span>
                            <span>Requested: <strong class="text-white-50">{{ $rep->created_at->format('M d, Y H:i') }}</strong></span>
                        </small>
                    </div>
                ...
                <div class="modal-footer bg-light border-top py-3 px-4 flex-shrink-0 d-flex justify-content-between align-items-center">
                    <div>
                        <button type="button" class="btn btn-outline-danger rounded-pill px-3.5 py-2 shadow-xs fw-semibold" data-bs-toggle="modal" data-bs-target="#rejectModal{{ $rep->id }}">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button type="submit" class="btn btn-warning text-dark rounded-pill px-4 py-2 fw-bold shadow-sm" onclick="return confirm('Send Replenishment #{{ $rep->request_no }} to Internal Audit Team?')">
                            <i class="fa-solid fa-paper-plane me-1.5"></i> Send to Audit Team
                        </button>
                    </div>
                </div>
            </form>
    ...
    <div class="modal fade" id="rejectModal{{ $rep->id }}" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <form method="POST" action="{{ url('/assigned-accounts/' . $rep->chart_of_account_id . '/replenishments/' . $rep->id . '/reject') }}" class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
                @csrf
                <div class="modal-header bg-danger text-white py-3 px-4 flex-shrink-0">
