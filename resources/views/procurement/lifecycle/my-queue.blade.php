...
{{-- ═══════════════════════════════════════════════════════════
     ALL OFFICE REQUEST MODALS — rendered OUTSIDE the table
     so Bootstrap backdrop/z-index works correctly
═══════════════════════════════════════════════════════════ --}}
@foreach($myPrs as $pr)
    @if($pr->is_office_request)

    {{-- Send to PM Modal --}}
    @if(in_array($pr->status, ['approved', 'pending_store_review']))
    <div class="modal fade" id="sendToPmModal{{ $pr->id }}" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="POST" action="{{ \Illuminate\Support\Facades\Route::has('office-requests.send-to-pm') ? route('office-requests.send-to-pm', $pr) : url('/office-requests/' . $pr->id . '/send-to-pm') }}" class="modal-content border-0 shadow">
                ...
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title fw-bold"><i class="fas fa-paper-plane me-2"></i>Send to Purchase Manager (PM)</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
                    <p class="text-muted small mb-3">
                        Forward <strong>{{ $pr->pr_no }}</strong> ({{ $pr->office_purpose ?: 'Office Supplies' }}) to Purchase Manager for procurement sourcing &amp; buying.
                    </p>
                    <div class="mb-0">
                        <label class="form-label fw-semibold text-dark">Store Manager Note (አስተያየት)</label>
                        <textarea name="notes" class="form-control" rows="3" placeholder="e.g. Items out of stock. Please source from suppliers..."></textarea>
                    </div>
                </div>
    ...
    @if(in_array($pr->status, ['approved', 'pending_store_review']))
    <div class="modal fade" id="dispatchModal{{ $pr->id }}" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="POST" action="{{ \Illuminate\Support\Facades\Route::has('office-requests.store-dispatch') ? route('office-requests.store-dispatch', $pr) : url('/office-requests/' . $pr->id . '/store-dispatch') }}" class="modal-content border-0 shadow">
                ...
                <div class="modal-header bg-success text-white">
                    <h5 class="modal-title fw-bold"><i class="fas fa-boxes-packing me-2"></i>Dispatch from Store &amp; Send to Finance Head</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
                    <div style="background:#d1fae5;border:1px solid #34d399;border-radius:8px;padding:10px 14px;margin-bottom:14px;font-size:0.88rem;color:#065f46;line-height:1.5;">
                        ...
                        After confirming, <strong>{{ $pr->pr_no }}</strong> will be marked as <strong>issued from store</strong> and forwarded to <strong>Finance Head</strong> for expense recording &amp; payment. A Finance expense entry will be created automatically.
                    </div>
                    <div class="mb-0">
                        <label class="form-label fw-semibold text-dark">Dispatch Note (የመዝገብ ማስታወሻ)</label>
    ...
    <div class="modal fade" id="financeConfirmModal{{ $pr->id }}" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="POST" action="{{ \Illuminate\Support\Facades\Route::has('office-requests.finance-confirm') ? route('office-requests.finance-confirm', $pr) : url('/office-requests/' . $pr->id . '/finance-confirm') }}" class="modal-content border-0 shadow">
                ...
                <div class="modal-header text-white" style="background:#7c3aed;">
                    <h5 class="modal-title fw-bold"><i class="fas fa-file-invoice-dollar me-2"></i>Confirm Office Expense &amp; Mark Paid</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
                    <div style="background:#ede9fe;border:1px solid #7c3aed;border-radius:8px;padding:10px 14px;margin-bottom:14px;font-size:0.88rem;color:#5b21b6;line-height:1.5;">
                    ...
                    <div class="mb-0">
                        <label class="form-label fw-semibold text-dark">Finance Notes (ማስታወሻ)</label>
