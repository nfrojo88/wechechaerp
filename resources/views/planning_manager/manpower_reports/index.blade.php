                ...
                <div class="col-lg-4 mb-3 mb-lg-0">
                    <div class="row g-1 text-center">
                        ...
                        <div class="col-12 mt-1">
                            <span class="badge bg-primary px-3 py-1.5 fw-bold">
                                <i class="fa-solid fa-users me-1"></i>{{ $report->total_present }} Total Present
                            </span>
                            <div class="small text-muted mt-1" style="font-size:0.72rem;">
                                <span class="text-warning fw-semibold">{{ $report->company_workers_count }} Company</span>
                                &bull;
                                <span class="text-info fw-semibold">{{ $report->subcontractor_workers_count }} Subcon</span>
                            </div>
                        </div>
                        @if((!empty($report->roles_breakdown) && is_array($report->roles_breakdown)) || (!empty($report->subcontractors_breakdown) && is_array($report->subcontractors_breakdown)))
                        <div class="col-12 mt-2 text-center">
                            <a class="btn btn-xs btn-outline-primary py-0 px-2 small shadow-sm" data-bs-toggle="collapse" href="#breakdown_{{ $report->id }}" role="button" aria-expanded="false" style="font-size:0.75rem;">
                                <i class="fa-solid fa-list-check me-1"></i>Detailed Breakdown
                            </a>
                            <div class="collapse mt-2 text-start" id="breakdown_{{ $report->id }}">
                                <div class="p-2 rounded bg-white border small shadow-sm">
                                    ...
                                    <div class="fw-bold text-dark mb-1" style="font-size:0.75rem;"><i class="fa-solid fa-users-gear text-warning me-1"></i>Company Labour:</div>
                                    ...
                                    @if(!empty($report->subcontractors_breakdown) && is_array($report->subcontractors_breakdown))
                                    <div class="fw-bold text-dark mb-1" style="font-size:0.75rem;"><i class="fa-solid fa-handshake text-info me-1"></i>Subcontractors:</div>
                                                ...
                                                @if(!empty($sub['roles']) && is_array($sub['roles']))
                                                    <span class="text-muted ms-1" style="font-size:0.7rem;">
                                                        ...
                                                        @foreach($sub['roles'] as $r)
                                                            {{ $r['role_name'] ?? 'Role' }}: {{ $r['count'] ?? 0 }}{{ !$loop->last ? ', ' : '' }}
                                                        @endforeach
                                                        ]
                                                    </span>
                                                @elseif(!empty($sub['role_name']))
